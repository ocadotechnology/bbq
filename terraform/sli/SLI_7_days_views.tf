resource "google_bigquery_table" "data_modified_6_days_ago_view" {
  ...
  table_id = "data_modified_6_days_ago"
  description = "All tables which have modifications before 6 days ago"

  view {
    query = <<EOF
            #legacySQL
              --  Shows all tables which have modifications before 6 days ago
            SELECT * FROM (
              ...
              FROM (
                ...
                FROM
                  ...
                  WHERE
                    _PARTITIONTIME BETWEEN TIMESTAMP(DATE_ADD(NOW(), -8, "DAY")) AND TIMESTAMP(DATE_ADD(NOW(), -4, "DAY"))
                    AND lastModifiedTime <= TIMESTAMP(DATE_ADD(NOW(), -6, "DAY"))
...
resource "google_bigquery_table" "SLI_7_days_view" {
  ...
  view {
    query = <<EOF
          ...
          FROM
            [${google_bigquery_table.data_modified_6_days_ago_view.id}] as census
          LEFT JOIN (
            SELECT
          ...
          WHERE
            ...
            AND numRows != 0
            AND IFNULL(last_backups.backup_created, MSEC_TO_TIMESTAMP(0)) < TIMESTAMP(DATE_ADD(CURRENT_TIMESTAMP(), -6 , "DAY"))
